...
import { cloudConfigured, getSession, loadCloudData, login, saveCloudData, exportarBackupCGL, importarBackupCGL, salvarDadosLocaisDireto, type AuthSession, type CloudData } from "./cloud";
...
.funcionariosContador{
  ...
  line-height:1;
  letter-spacing:.2px;
  box-shadow:0 6px 18px rgba(22,127,232,.10);
}
.funcionarioNumero{
  min-width:38px;
  height:38px;
...
function App() {
  ...
  // Obra que aparece manualmente em destaque no Dashboard.
  // Fica salva apenas no dispositivo, sem alterar os dados das obras.
  const [obraDestaqueManualId, setObraDestaqueManualId] = useState<number | null>(() => {
    const salvo = localStorage.getItem("obraDestaqueManualId");
    if (salvo === null) return null;
    const numero = Number(JSON.parse(salvo));
    return Number.isFinite(numero) ? numero : null;
  });
  const [seletorObraDashboardAberto, setSeletorObraDashboardAberto] = useState(false);
  ...
  // Tela cheia: o navegador só permite esconder a barra de status/navegação
  // depois de uma interação real do usuário. O primeiro toque/clique no CGL
  // tenta ativar o modo imersivo sem criar botão ou alterar o layout.
  useEffect(() => {
    let tentouEntrarEmTelaCheia = false;

    const entrarEmTelaCheia = async () => {
      if (tentouEntrarEmTelaCheia) return;
      tentouEntrarEmTelaCheia = true;
      ...
      try {
        ...
        } else if (typeof elemento.webkitRequestFullscreen === "function") {
          await elemento.webkitRequestFullscreen();
        }
      } catch {
        // Alguns navegadores/WebViews não permitem fullscreen. Nesse caso
        // o CGL continua normalmente, sem alterar nenhuma função.
      }
    };

    document.addEventListener("pointerdown", entrarEmTelaCheia, { once: true });

    return () => {
      document.removeEventListener("pointerdown", entrarEmTelaCheia);
    };
  }, []);
  ...
  const baixarOuCompartilharBackup = async () => {
    ...
    try {
      // Gera o backup diretamente em memória. Não grava uma segunda cópia
      // no localStorage, o que evita problemas com backups que incluem fotos.
      const conteudo = exportarBackupCGL(dadosRef.current);
      arquivo = new File([conteudo], nomeBackup(), {
        type: "application/octet-stream",
      });

      // No Android/Chrome, tenta primeiro o compartilhamento nativo.
      // Não dependemos de canShare(), pois alguns navegadores Android
      // retornam false mesmo conseguindo compartilhar o arquivo.
      if (
        ...
        try {
          ...
          // O usuário cancelou: não cria download extra.
          if (
            erroCompartilhamento instanceof DOMException &&
            erroCompartilhamento.name === "AbortError"
          ) {
            return;
          }
          // Se o navegador não aceitar o compartilhamento de arquivo,
          // continua para o download normal.
          console.warn(
            "Compartilhamento nativo indisponível; usando download.",
            erroCompartilhamento
          );
        }
      }

      // Fallback: funciona no PC e em navegadores Android que não suportam
      ...
      // Mantém a URL disponível tempo suficiente para o Android concluir
      // o download antes de liberar o objeto em memória.
      window.setTimeout(() => {
      ...
      console.error("Erro ao criar/compartilhar backup do CGL:", erro);
      ...
      window.alert(
        `Não foi possível gerar o backup. ${mensagem}\n\n` +
          "Nenhum dado do CGL foi apagado."
      );
    } finally {
      if (url) URL.revokeObjectURL(url);
    }
...
const adicionarEtapa = () => {
  ...
  const atividades = [
    ...
    .slice(0, 5);
  // A escolha manual do destaque é salva localmente. Se a obra escolhida for
  // concluída, o Dashboard volta automaticamente para a primeira obra ativa.
  useEffect(() => {
    if (obraDestaqueManualId === null) {
      localStorage.removeItem("obraDestaqueManualId");
    } else {
      localStorage.setItem("obraDestaqueManualId", String(obraDestaqueManualId));
    }
  }, [obraDestaqueManualId]);

  const obrasAtivasDashboard = obras.filter((obra) => obra.status !== "Concluído");
  const obraManualValida = obrasAtivasDashboard.find((obra) => obra.id === obraDestaqueManualId);
  const obraDestaque = obraManualValida || obrasAtivasDashboard[0];
    ...
    <div key={dashboardAtualizacaoKey}>
      ...
      <div className="dashboardHero">
        <section className="projectCard">
          ...
          {obrasAtivasDashboard.length >= 1 && (
            <div style={{position:"absolute",top:10,right:10,zIndex:20}}>
              <button
                type="button"
                onClick={() => setSeletorObraDashboardAberto((aberto) => !aberto)}
                aria-label="Trocar obra em destaque"
                title="Trocar obra em destaque"
                style={{
                  border:"1px solid rgba(21,151,245,.45)",
                  ...
                  fontWeight:900,
                  lineHeight:1,
                  boxShadow:"0 4px 12px rgba(36,59,83,.22)",
                  backdropFilter:"blur(6px)"
                }}
              >↔</button>

              {seletorObraDashboardAberto && (
                <div
                  style={{
                    position:"absolute",
                    right:0,
                ...
                >
                  <div style={{padding:"6px 8px 7px",fontSize:10,fontWeight:800,color:"#60758a",textTransform:"uppercase",letterSpacing:".05em"}}>
                    Trocar obra do Dashboard
                  </div>
                  {obrasAtivasDashboard.map((obra) => {
                    const selecionada = obra.id === obraDestaque?.id;
                    return (
                      <button
                        key={obra.id}
                        type="button"
                        onClick={() => {
                          setObraDestaqueManualId(obra.id);
                          setSeletorObraDashboardAberto(false);
                        }}
                        style={{
                          width:"100%",
                          textAlign:"left",
                          ...
                          padding:"8px 9px",
                          marginBottom:3,
                          background:selecionada ? "#e9f4fb" : "transparent",
                          color:"#24384a",
                          cursor:"pointer",
                          fontSize:11,
    ...
    <>
      ...
      <section className="panel funcionariosPanel">
        <div className="funcionariosTop">
          ...
          <div
            className="funcionariosContador"
            aria-label={`${pessoas.length} funcionário${pessoas.length === 1 ? "" : "s"}`}
            title={`${pessoas.length} funcionário${pessoas.length === 1 ? "" : "s"}`}
          >
            {pessoas.length < 100 ? String(pessoas.length).padStart(2, "0") : pessoas.length}
          </div>
        </div>
        ...
        {pessoas.length === 0 ? (
          ...
          <div className="funcionariosGrid">
            {pessoas.map((pessoa, index) => {
              ...
              const numeroFuncionario = index + 1;
              ...
              return (
                <article className="funcionarioCard" key={pessoa.id}>
                  <div className="funcionarioCardHeader">
                    ...
                    <span className="funcionarioNumero" aria-label={`Funcionário número ${numeroFuncionario}`}>
                      {numeroFuncionario < 100 ? String(numeroFuncionario).padStart(2, "0") : numeroFuncionario}
                    </span>
                  </div>
              ...
              <div className="equipeGrid">
                {pessoas.map((pessoa, index) => {
                  ...
                  return (
                    ...
                    >
                      <label>
                        <span className="equipeFuncionarioNome">
                          <input
                            type="checkbox"
                            checked={selecionado}
                            onChange={() =>
                        ...
                        <span
                          className="funcionarioNumero equipeFuncionarioNumero"
                          aria-label={`Funcionário número ${numeroFuncionario}`}
                        >
                          {numeroFuncionario < 100
                            ? String(numeroFuncionario).padStart(2, "0")
                            : numeroFuncionario}
                        </span>
                      </label>
